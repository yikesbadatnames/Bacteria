...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var filteredMeta = metadata.filter(s => s.id == sample);
    ...
    var topOtuIds = otuId.slice(0,10).reverse();
    ...
    var barData = [{
     ...
     console.log(barData); 
     var barLayout = {
       title: `${sample}'s Top 10 Cultures`
    ...
    var bubbleLayout = {
      title: `${sample}'s Biodiversity Spread`
    ...
    var gaugeData = [{
      ...
      value: wfreq,
      type: "indicator",
      title: {text: "Sanitation Freq <br> Scrubs per Week"},
      gauge: {
        axis: { range: [null, 10] },
        bar: { color: "black" },
        steps: [
